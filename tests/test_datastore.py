...
def test_multi_device_one_index(tmp_path):
    """Several samples share ONE data_root + ONE index; device = the sample
    name, and every device-keyed query is scopeable by it."""
    from scqo import DataStore
    ...
    assert {r["run_id"] for r in store.find_runs()} == {ra["run_id"],
                                                        rb["run_id"]}
